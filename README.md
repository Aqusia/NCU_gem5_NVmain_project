...
checkout. `setup.sh` does exactly that for you.
...
> the image instead of via `setup.sh`, add `COPY . /work`, `RUN /work/setup.sh`
...
```bash
./setup.sh        # clone + overlay + build into ./_work
source env.sh
./run_all.sh
```
...
flags. After `source env.sh`, just call `gem5sim …`.
...
| Variable | Meaning | Default after `source env.sh` |
|---|---|---|
